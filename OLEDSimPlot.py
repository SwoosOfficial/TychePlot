...
class Stack:
    ...
    def __init__(self, Materials, colors, colorsMetal):
        ...
        for m,b in zip(self.Materials, self.B):
            ...
            if not m.metallic:
                ...
                if not plot.colorGrad:
                    ax.plot(np.asarray(m.x)*plot.x_scale,m.y*e**-1, color=self.Cz[m.id], label=m.name)
                    ax.plot(np.asarray(m.x)*plot.x_scale,m.y_2*e**-1, color=self.Cz[m.id])
                    ...
                    if type(m) is DopedOrganic:
                        ...
                        ax.plot(np.asarray(m.x)*plot.x_scale,m.y_3*e**-1, color=self.Cz[m.id2], label=m.name2)
                        ax.plot(np.asarray(m.x)*plot.x_scale,m.y_4*e**-1, color=self.Cz[m.id2])
                        ax.fill_between(np.asarray(m.x)*plot.x_scale,m.y_3*e**-1,m.y_4*e**-1, facecolor=self.Cz[m.id2],interpolate=False, alpha=plot.alphaBG)
                        ax.fill_between(np.asarray(m.x)*plot.x_scale,m.y_2*e**-1,m.y_4*e**-1, facecolor=self.Cz[m.id],interpolate=False, alpha=plot.alphaBG)
                        ax.fill_between(np.asarray(m.x)*plot.x_scale,m.y*e**-1,m.y_3*e**-1, facecolor=self.Cz[m.id],interpolate=False, alpha=plot.alphaBG)
                    else:
                        ax.fill_between(np.asarray(m.x)*plot.x_scale,m.y*e**-1,m.y_2*e**-1, facecolor=self.Cz[m.id],interpolate=False, alpha=plot.alphaBG)
                else:
        ...
        for m,b in zip(self.Materials, self.B):
            if m.metallic:
                if b==-2 and plot.metalContactAnode:
                    ...
                    ax.plot(np.asarray(m.x)*plot.x_scale,np.asarray(m.y)*e**-1, color=self.C[b-1], label=m.name)
                    ax.fill_between(np.asarray(m.x)*plot.x_scale,x_min,np.asarray(m.y)*e**-1, interpolate=True, facecolor=self.C[b-1], alpha=plot.alphaMBG)
                    ...
                    m.vacY=[d-m.CBLike for d in m.y]
                    ax.plot(np.asarray(m.x)*plot.x_scale,np.asarray(m.y)*e**-1, color=self.C[b-1], label=m.name)
                    ax.fill_between(np.asarray(m.x)*plot.x_scale,plot.x_min,np.asarray(m.y)*e**-1, interpolate=True, facecolor=self.C[b-1], alpha=plot.alphaMBG)
                    ax.text((m.x[(len(m.x)-1)//2]+m.desc_x_offset)*plot.x_scale,(m.y[0]+plot.x_min*e)/2*e**-1+m.height,m.name, ha="center", va="center")
                    ax.plot(np.asarray(m.x)*plot.x_scale,np.asarray(m.vacY)*e**-1, "k-",label="Vacuum")
                else:
                    ...
                    if b==-2:
                        ax.plot(np.asarray(m.x)*plot.x_scale+0.25,np.asarray(m.y)*e**-1, color=self.C[b-1], label=m.name)
                        ax.fill_between(np.hstack([m.x,[m.x[-1]+0.5/plot.x_scale]])*plot.x_scale,plot.x_min,np.hstack([m.y,[m.y[-1]]])*plot.e**-1,interpolate=True, facecolor=self.C[b-1], alpha=plot.alphaMBG)
                    elif b==2:
                        ax.plot(np.asarray(m.x)*plot.x_scale-0.25,np.asarray(m.y)*e**-1, color=self.C[b-1], label=m.name)
                        ax.fill_between(np.hstack([[m.x[0]-0.5/plot.x_scale],m.x])*plot.x_scale,plot.x_min,np.hstack([[m.y[0]],m.y])*plot.e**-1,interpolate=True, facecolor=self.C[b-1], alpha=plot.alphaMBG)
                    else:
                        ax.plot(np.asarray(m.x)*plot.x_scale,np.asarray(m.y)*e**-1, color=self.C[b-1], label=m.name)
                        ax.fill_between(np.asarray(m.x)*plot.x_scale,plot.x_min,np.asarray(m.y)*plot.e**-1,interpolate=True, facecolor=self.C[b-1], alpha=0.3)
                    ax.text((m.x[(len(m.x)-1)//2]+m.desc_x_offset)*plot.x_scale,(m.y[0]+plot.x_min*e)/2*e**-1+m.height,m.name, ha="center", va="center")
    ...
    def createGradient(self, ax, m, plot, b=0):
        ...
        new_pre_cmp=np.array([[rgb[0]/255,rgb[1]/255,rgb[2]/255,new_color] for new_color in new_colors])
        new_cmp= ListedColormap(new_pre_cmp)
        extent1=(m.x[0]*plot.x_scale, m.x[-1]*plot.x_scale, (m.y[1]-m.sigma)*e**-1, (m.y[1]+m.sigma)*e**-1)
        extent2=(m.x[0]*plot.x_scale, m.x[-1]*plot.x_scale, (m.y_2[1]-m.sigma)*e**-1, (m.y_2[1]+m.sigma)*e**-1)
        ...
        if type(m) is DopedOrganic:
            ...
            #new_colors[127:]=np.amax(np.asarray([new_colors[127:],plot.alphaBG*np.ones(129)]), axis=0)
            new_pre_cmp_2=np.array([[rgb_2[0]/255,rgb_2[1]/255,rgb_2[2]/255,new_color] for new_color in new_colors_2])
            new_cmp_2= ListedColormap(new_pre_cmp_2)
            m.y_3=np.asarray(plot.V_1(m.x,m.CB2Like,plot.points(m.thickness,m.dim),sigma=m.sigma))+np.asarray(plot.V_2(m.x, b))+np.asarray(plot.V_3(m.x, m.x[0]))+np.asarray(plot.V_3(m.x,m.x[0], E=m.polarity))
            m.y_4=np.asarray(plot.V_1(m.x,m.VL2Like,plot.points(m.thickness,m.dim),sigma=m.sigma))+np.asarray(plot.V_2(m.x, b))+np.asarray(plot.V_3(m.x, m.x[0]))+np.asarray(plot.V_3(m.x,m.x[0], E=m.polarity))
            extent3=(m.x[0]*plot.x_scale, m.x[-1]*plot.x_scale, (m.y_3[1]-m.sigma)*e**-1, (m.y_3[1]+m.sigma)*e**-1)
            extent4=(m.x[0]*plot.x_scale, m.x[-1]*plot.x_scale, (m.y_4[1]-m.sigma)*e**-1, (m.y_4[1]+m.sigma)*e**-1)
