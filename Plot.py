...
class Plot:
        ...
        if not errorData:
            ...
            devDataList = [
                np.sqrt(
                    np.sum(
                        [
                            np.square(np.subtract(data, tempData.getData()))
                            for tempData in deviceData
                        ],
                        axis=0,
                    )
                    / len(deviceData)
                )
                for data, deviceData in zip(avgDataList, dataList)
            ]
            avgData = [
                Data(data, desc=desc, xCol=self.xCol, yCol=self.showCol)
                for data, desc in zip(avgDataList, descList)
            ]
            devData = [
                Data(data, xCol=self.xCol, yCol=self.showCol) for data in devDataList
            ]
        else:
            avgData = []
            devData = []
    ...
    def processMedian(self, dataList=None):
        ...
        if not errorData:
            ...
            medList = [
                [medColList[m][n] for m in range(0, qtyCol)]
                for n in range(0, len(dataList))
            ]
            ...
            devDataList = [
                ...
                for data, deviceData in zip(medDataList, dataList)
            ]
            medData = [
                Data(data, desc=desc, xCol=self.xCol, yCol=self.showCol)
                for data, desc in zip(medDataList, descList)
            ]
            devData = [
                Data(data, xCol=self.xCol, yCol=self.showCol) for data in devDataList
            ]
    ...
    def processCustomMedian(self, dataList):
        qtyCol = len(dataList[0][0].getData()[0])
        dataColList = [
            [[data.getData()[:, m] for data in deviceData] for deviceData in dataList]
            for m in range(0, qtyCol)
        ]
        medColList = [
            [np.median(element, axis=0) for element in column] for column in dataColList
        ]
        medList = [
            [medColList[m][n] for m in range(0, qtyCol)]
            for n in range(0, len(dataList))
    ...
    def calcLogErr(self, expectData, deviaData):
        ...
        try:
            minData = [
                np.amin([data.getData() for data in deviceData], axis=0)
                for deviceData in self.dataList
            ]
            maxData = [
                np.amax([data.getData() for data in deviceData], axis=0)
                for deviceData in self.dataList
            ]
            symErr = [
                (np.absolute(expect.getData() - devia.getData()))
                for devia, expect in zip(deviaData, expectData)
            ]
            minErr = [
                np.absolute(expect.getData() - mind)
                for devia, expect, mind in zip(deviaData, expectData, minData)
            ]
            maxErr = [
                np.absolute(expect.getData() - maxi)
                for devia, expect, maxi in zip(deviaData, expectData, maxData)
            ]
            logErrMin = [
                np.nan_to_num(np.minimum(err, mind), nan=0, posinf=0, neginf=0)
                for err, mind in zip(symErr, minErr)
            ]
            logErrMax = [
                np.nan_to_num(np.minimum(err, maxi), nan=0, posinf=0, neginf=0)
                for err, maxi in zip(symErr, maxErr)
            ]
            # if any error is as high that the bar would reach zero, set it to zero
            logErrMin = np.asarray(
                [
                    np.asarray(
                        [
                            np.asarray(
                                [
                                    errv if value > errv else 0.0
                                    for errv, value in zip(errv_row, value_row)
                                ],
                                dtype=object,
                            )
                            for errv_row, value_row in zip(err, expect.getData())
                        ],
                        dtype=object,
                    )
                    for err, expect in zip(logErrMin, expectData)
                ],
                dtype=object,
            )
            values = np.asarray(
                [
                    np.asarray(
                        [
                            np.asarray(
                                [value for errv, value in zip(errv_row, value_row)],
                                dtype=object,
                            )
                            for errv_row, value_row in zip(err, expect.getData())
                        ],
            ...
            errvs = np.asarray(
                [
                    np.asarray(
                        [
                            np.asarray(
                                [errv for errv, value in zip(errv_row, value_row)],
                                dtype=object,
                            )
                            for errv_row, value_row in zip(err, expect.getData())
                        ],
    ...
    def concentenate_data(self, dataList=None):
        ...
        for dataSubList in dataList:
            ...
            try:
                device_data = Data(
                    np.vstack(dataArray), xCol=self.xCol, yCol=self.showCol
                )
                device_data.removeDoubles()
                new_dataList.append(device_data)
                new_devia_dataList.append(
                    Data(
                        np.zeros(device_data.getData().shape),
                        xCol=self.xCol,
                        yCol=self.showCol,
                    )
                )
    ...
    def processPlotSub(
        ...
        try:
            ...
            if self.append_col_in_label:
                labelY = [
                    labels[n] + " " + self.showColLabel[singleShowCol]
                    for singleShowCol in self.showCol
                ]
            else:
                labelY = labels[n]
        if self.show[n][0]:
            if self.errors[n][0]:
                try:
                    try:
                        yerr = [
                            self.logErr[self.errorTypeDown][n][:, showCol - 1],
                            self.logErr[self.errorTypeUp][n][:, showCol - 1],
                        ]
                    except IndexError:
                        yerr = None
                    AX = ax.errorbar(
                        *expectData[n].getSplitData2D(xCol=xCol, yCol=showCol),
                        yerr=yerr,
                        c=ax1color[n],
                        capsize=self.capsize,
                        capthick=self.capthick,
                        ls=linestyles[n],
                        marker=markerstyles[n],
                        label=labelY,
                        errorevery=self.showErrorOnlyEvery[n],
                        fillstyle=fillstyles[n],
                    )
                except TypeError:
                    for singleShowCol, singleLabelY in zip(showCol, labelY):
                        AX = ax.errorbar(
                            *expectData[n].getSplitData2D(
                                xCol=xCol, yCol=singleShowCol
                            ),
                            yerr=[
                                self.logErr[self.errorTypeDown][n][:, showCol - 1],
                                self.logErr[self.errorTypeUp][n][:, showCol - 1],
            ...
            if self.fitList is not None and self.fitterList[n] is not None:
                if type(self.fitterList[n]) is list:
                    for fitter in self.fitterList[n]:
                        FIT = ax.errorbar(
                            *fitter.CurveData.getSplitData2D(),
                            c=self.fitColors[n],
                            ls=self.fitLs,
                            label=self.fitLabels[n],
                            alpha=self.fitAlpha,
                        )
                else:
                    FIT = ax.errorbar(
                        *self.fitterList[n].CurveData.getSplitData2D(),
                        c=self.fitColors[n],
                        ls=self.fitLs,
                        label=self.fitLabels[n],
                        alpha=self.fitAlpha,
        ...
        if self.show[n][1] and self.showCol2 != 0:
            if self.customLabelAx2 is not None:
                ...
                try:
                    ...
                    labelZ = [
                        labels[n] + " " + self.showColLabel[singleShowCol2]
                        for singleShowCol2 in showCol2
                    ]
            if self.errors[n][1]:
                try:
                    AX2 = ax2.errorbar(
                        *expectData[n].getSplitData2D(xCol=xCol, yCol=showCol2),
                        yerr=[
                            self.logErr[self.ax2errorTypeDown][n][:, showCol2 - 1],
                            self.logErr[self.ax2errorTypeUp][n][:, showCol2 - 1],
                        ],
                        capsize=self.capsize,
                        capthick=self.capthick,
                        c=ax2color[n],
                        ls=ax2linestyles[n],
                        marker=ax2markerstyles[n],
                        label=labelZ,
                        errorevery=self.showErrorOnlyEvery[n],
                        fillstyle=ax2fillstyles[n],
                    )
                except TypeError:
                    for singleShowCol2, singleLabelZ in zip(showCol2, labelZ):
                        AX2 = ax2.errorbar(
                            *expectData[n].getSplitData2D(
                                xCol=xCol, yCol=singleShowCol2
                            ),
                            yerr=[
                                self.logErr[self.ax2errorTypeDown][n][:, showCol2 - 1],
                                self.logErr[self.ax2errorTypeUp][n][:, showCol2 - 1],
            ...
            if self.fitList is not None and self.fitterList[n] is not None:
                if type(self.fitterList[n]) is list:
                    for fitter in self.fitterList[n]:
                        ...
                        FIT2 = ax2.errorbar(
                            *fitter.CurveData.getSplitData2D(),
                            c=self.fitColors[n + colorOffset],
                            ls=self.fitLs,
                            label=self.fitLabels[n],
                            alpha=self.fitAlpha,
                        )
    ...
    def doPlot(self):
        ...
        if not self.no_plot:
            self.fig, self.ax = self.newFig(
                customFontsize=self.customFontsize,
                font=self.font,
                axRect=self.axRect,
            ...
            try:
                if self.showColAxType[self.showCol] == "log":
                    ax.set_yscale(
                        "log", nonpositive="clip"
                    )  # , basex=10, subsy=[2,3,4,5,6,7,8,9])
            except TypeError:
            ...
            if (
                True in [a[1] for a in self.show]
                and self.ax2Labels
                and self.showCol2 != 0
            ):
            ...
            if self.fitList is not None:
                index_list = [
                    True if x not in self.fitLabels + self.partialFitLabels else False
                    for x in labels
                ]  #
                ...
                if self.showFitInLegend:
                    labels += [
                        l for l, index in zip(orig_labels, index_list) if not index
                    ]
                    handles += [
                        h for h, index in zip(orig_handles, index_list) if not index
                    ]
            if self.legendBool:
                leg = ax.legend(
                    handles, labels, loc=self.legLoc, numpoints=1, facecolor="none"
                )
                ...
                for o in range(0, len(expectArray[a])):  # o=element
                    if noError:
                        ...
                        line += (
                            str(expectArray[a][o])
                            + colSep
                            + str(errData[errorTypes[o]][l][a][o])
                            + colSep
                        )
                line += "\n"
                file.write(line)
